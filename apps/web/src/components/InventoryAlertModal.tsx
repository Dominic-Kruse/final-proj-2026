...
type ModalType = "low-stock" | "expired";
...
const CONFIG = {
  "low-stock": {
    ...
    summary: "These products are at or below their reorder level. Restock soon to avoid running out.",
  ...
  "expired": {
    ...
    summary: "These batches have passed their expiry date. Remove them from the shelf immediately.",
    ...
    detail: (item: CatalogItem) => {
      const expiredCount = item.batches.filter(b => new Date(b.expiryDate) < new Date()).length;
      ...
      >
        ...
        <div className="flex items-start justify-between p-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 max-w-md">{config.summary}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-4">
          {isLoading ? (
            ...
            <p className="text-center text-slate-400 py-12">{config.emptyMessage}</p>
          ) : (
              ...
              <tbody>
                {filtered.map(item => (
                  <tr key={item.productId} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className="py-3 font-medium text-slate-800">{item.productDetails}</td>
                    <td className="py-3 text-slate-500">{item.genericName} {item.dosage}</td>
                    <td className="py-3">{config.badge(item)}</td>
                    <td className="py-3 text-right text-slate-400">{config.detail(item)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
